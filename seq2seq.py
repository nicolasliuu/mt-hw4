...
class EncoderRNN(nn.Module):
    ...
    def forward(self, input, hidden):
        ...
        # GRU in bidirectional mode returns outputs for both directions
        output, hidden = self.gru(embedded, hidden)
        # Sum the outputs from both directions
        output = output[:, :, :self.hidden_size] + output[:, :, self.hidden_size:]
        return output, hidden

    def init_hidden(self):
        # For bidirectional GRU, number of layers * num_directions
...
class AttnDecoderRNN(nn.Module):
    ...
    def forward(self, input, hidden, encoder_outputs):
        ...
        # Concatenate embedded and hidden state for attention
...
def train_step(input_tensor, target_tensor, encoder, decoder, encoder_optimizer, decoder_optimizer, criterion, max_length=MAX_LENGTH):
    ...
    # Initialize encoder outputs for bidirectional encoder
    ...
    # Use the sum of the forward and backward hidden states as the initial decoder hidden state
    decoder_hidden = torch.sum(encoder_hidden, dim=0).unsqueeze(0)
    ...
    with torch.no_grad():
        ...
        for di in range(max_length):
            ...
            if topi.item() == EOS_index:
                ...
                decoded_words.append(tgt_vocab.index2word.get(topi.item(), EOS_token))
...
def compute_bleu(encoder, decoder, pairs, src_vocab, tgt_vocab, max_length=MAX_LENGTH):
    references = []
    hypotheses = []

    for pair in pairs:
        reference = clean(pair[1]).split()
        translation, _ = translate(encoder, decoder, pair[0], src_vocab, tgt_vocab, max_length)
        hypothesis = clean(' '.join(translation)).split()
...
def main():
    ...
    if args.load_checkpoint is not None:
        ...
        encoder_optimizer.load_state_dict(state['enc_opt_state'])
        decoder_optimizer.load_state_dict(state['dec_opt_state'])
    ...
    while iter_num < args.n_iters:
        ...
        loss = train_step(input_tensor, target_tensor, encoder, decoder, encoder_optimizer, decoder_optimizer, criterion, max_length=MAX_LENGTH)
        ...
        if iter_num % args.print_every == 0:
            ...
            logging.info('Time since start: %.2f sec | Iteration: %d/%d (%.2f%%) | Loss_avg: %.4f',
                         elapsed,
                         iter_num,
                         args.n_iters,
            ...
            # Compute BLEU score on the development set
            bleu_score = compute_bleu(encoder, decoder, dev_pairs, src_vocab, tgt_vocab)
            logging.info('BLEU score on dev set: %.4f', bleu_score)
