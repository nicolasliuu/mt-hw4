...
class MyLSTMCell(nn.Module):
    ...
    def forward(self, x_t, h_prev, c_prev):
        ...
        g_t = torch.tanh(self.W_c(x_t) + self.U_c(h_prev))
...
class EncoderRNN(nn.Module):
    ...
    def __init__(self, input_size, hidden_size):
        ...
        self.lstm_cell_forward = MyLSTMCell(hidden_size, hidden_size)
        self.lstm_cell_backward = MyLSTMCell(hidden_size, hidden_size)
    ...
    def forward(self, input, hidden):
        ...
        for i in range(seq_len):
            x_t = embedded[i]
            h_f, c_f = self.lstm_cell_forward(x_t, h_f, c_f)
            output_f.append(h_f)

        for i in reversed(range(seq_len)):
            x_t = embedded[i]
            h_b, c_b = self.lstm_cell_backward(x_t, h_b, c_b)
            output_b.insert(0, h_b)

        for i in range(seq_len):
            outputs[i] = torch.cat((output_f[i], output_b[i]), dim=1).squeeze(0)

        return outputs, (h_f, c_f)

    def get_initial_hidden_state(self):
        return None  # Not used since we initialize h_f, h_b to zeros in forward()
...
class AttnDecoderRNN(nn.Module):
    ...
    def __init__(self, hidden_size, output_size, dropout_p=0.1, max_length=MAX_LENGTH):
        ...
        self.lstm_cell = MyLSTMCell(hidden_size + hidden_size * 2, hidden_size)
    ...
    def forward(self, input, hidden, encoder_outputs):
        # input shape: (1, 1)
        embedded = self.embedding(input).view(1, -1)  # Reshape to (1, hidden_size)
        embedded = self.dropout(embedded)  # Shape: (1, hidden_size)

        s_prev = hidden[0]  # Shape: (1, hidden_size)
        s_prev_expanded = s_prev.expand(encoder_outputs.size(0), -1)  # Shape: (seq_len, hidden_size)

        e_ij = self.attn_v(torch.tanh(
            self.attn_Wa(s_prev_expanded) + self.attn_Ua(encoder_outputs)))  # Shape: (seq_len, 1)

        attn_weights = F.softmax(e_ij.squeeze(1), dim=0).unsqueeze(0).unsqueeze(1)  # Shape: (1, 1, seq_len)

        c_i = torch.bmm(attn_weights, encoder_outputs.unsqueeze(0))  # Shape: (1, 1, hidden_size * 2)
        c_i = c_i.squeeze(0)  # Shape: (1, hidden_size * 2)

        lstm_input = torch.cat((embedded, c_i), dim=1)  # Shape: (1, hidden_size + hidden_size * 2)

        h_i, c_i = self.lstm_cell(lstm_input, hidden[0], hidden[1])  # Both h_i and c_i have shape (1, hidden_size)

        output = self.out(h_i)  # Shape: (1, output_size)
        log_softmax = F.log_softmax(output, dim=1)

        # Return attn_weights without the extra dimensions
        return log_softmax, (h_i, c_i), attn_weights.squeeze(0)
...
def translate(encoder, decoder, sentence, src_vocab, tgt_vocab, max_length=MAX_LENGTH):
    ...
    with torch.no_grad():
        ...
        for di in range(max_length):
            decoder_output, decoder_hidden, decoder_attention = decoder(
                decoder_input, decoder_hidden, encoder_outputs)
            # decoder_attention is of shape (input_length,)
            # Assign it directly to decoder_attentions[di]
