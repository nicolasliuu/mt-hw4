...
SOS_index = 0
EOS_index = 1
PAD_index = 2  # Added PAD token index
MAX_LENGTH = 15
...
def tensor_from_sentence(vocab, sentence):
    ...
    for word in sentence.strip().split():
        try:
            ...
            indexes.append(vocab.word2index.get("<UNK>", PAD_index))  # Handle unknown words
...
class EncoderRNN(nn.Module):
    ...
    def forward(self, input_seqs, input_lengths):
        # input_seqs: (batch_size, max_seq_len)
        ...
        outputs, _ = nn.utils.rnn.pad_packed_sequence(packed_outputs, batch_first=True)  # (batch_size, seq_len, hidden_size*2)
...
class AttnDecoderRNN(nn.Module):
    ...
    def __init__(self, hidden_size, output_size, dropout_p=0.1, max_length=MAX_LENGTH):
        ...
        self.hidden_size = hidden_size  # Should be the same as in EncoderRNN
        ...
        # Since encoder outputs have hidden_size * 2, we adjust the input size accordingly
    ...
    def forward(self, input, hidden, encoder_outputs, mask):
        # input: (batch_size)
        # hidden: (h_t, c_t), each of shape (batch_size, hidden_size)
        ...
        s_prev = hidden[0]  # h_i: (batch_size, hidden_size)
        s_prev_expanded = s_prev.unsqueeze(1)  # (batch_size, 1, hidden_size)
        ...
        c_i = torch.bmm(attn_weights.unsqueeze(1), encoder_outputs).squeeze(1)  # (batch_size, hidden_size * 2)

        lstm_input = torch.cat((embedded, c_i), dim=1)  # (batch_size, hidden_size + hidden_size * 2)

        h_i, c_i = self.lstm_cell(lstm_input, hidden)  # h_i, c_i: (batch_size, hidden_size)

        output = self.out(h_i)  # (batch_size, output_size)
...
def train_batch(input_tensor, target_tensor, input_lengths, target_lengths, encoder, decoder, optimizer, criterion, max_length=MAX_LENGTH):
    ...
    encoder_mask = create_masks(input_lengths, encoder_outputs.size(1))  # (batch_size, seq_len)
    ...
    target_mask = create_masks(target_lengths, max_target_length)  # (batch_size, max_target_length)
...
def translate(encoder, decoder, sentence, src_vocab, tgt_vocab, max_length=MAX_LENGTH, beam_width=5):
    ...
    """
    # Switch the encoder and decoder to eval mode to disable dropout
    encoder.eval()
    decoder.eval()
    ...
    with torch.no_grad():
        ...
        encoder_mask = create_masks(input_length, encoder_outputs.size(1))  # Shape: (1, seq_len)

        # Initialize the beam with the start token
        decoder_hidden = decoder.get_initial_hidden_state(1)

        # Beam search variables
        beam = [([SOS_index], decoder_hidden, 0.0)]  # Each element is (sequence, hidden_state, score)
        ...
        for _ in range(max_length):
            ...
            for seq, hidden, score in beam:
                decoder_input = torch.tensor([seq[-1]], device=device)  # Shape: (1,)
            ...
            # Sort new beam candidates by score and keep the top beam_width ones
            new_beam = sorted(new_beam, key=lambda x: x[2], reverse=True)[:beam_width]
            beam = new_beam

            # If all sequences in the beam have reached EOS, break
        ...
        # If no completed sequences, use the current beam
        ...
        decoded_words = [tgt_vocab.index2word.get(idx, "<UNK>") for idx in best_sequence[1:]]  # Exclude SOS

        # Since attention is more complex with beam search, we can skip returning attentions or modify accordingly
        attentions = None  # Modify if you wish to compute attentions
...
def show_attention(input_sentence, output_words, attentions, save_path='attention.png'):
    ...
    # Convert attentions to numpy for plotting
    attentions = attentions.cpu().numpy()

    # Ensure attentions have the correct shape
    ...
    # Prepare the input and output words for labeling
    input_words = input_sentence.strip().split(' ') + [EOS_token]
    output_words = output_words  # Includes <EOS> if present

    # Set up the x and y ticks to correspond to input and output words
...
def main():
    ...
    while iter_num < n_iters:
        ...
        for batch_start in range(0, len(train_pairs), batch_size):
            ...
            input_padded = pad_sequences(input_tensors, max_input_length)  # (batch_size, max_input_length)
            target_padded = pad_sequences(target_tensors, max_target_length)  # (batch_size, max_target_length)
            ...
            if iter_num % args.print_every == 0:
                ...
                # In main function, during validation or testing
